...
class Rectification():
    ...
    def obj2(self, X, Y1,N,dt,notNan, w,l,lam1,lam2,lam3,lam4,lam5):
        """The cost function
    ...
    def estimate_dimension(self, df):
        '''                        
        estimate the length and width without state regularization
        '''             
    ...
    def rectify_single_camera_2(self, df,args):
        '''                        
        df: a single track in one camera view
        rectify given w and l
        '''   
        w,l = self.estimate_dimension(df)  
        print(w,l)        
        ...
        Y0 = self.generate(w,l,x0, y0, theta0,v0)
        diff = Y1-Y0[notNan,:]
        ...
        lams = (100,0,0,0,0)
        minimizer_kwargs = {"method":"L-BFGS-B", "args":(Y1,N,dt,notNan,w,l,*lams),'bounds':bnds,'options':{'disp': False}}
        res = basinhopping(self.obj2, X0, minimizer_kwargs=minimizer_kwargs,niter=0)
        print(res.fun/lams[0], res.message)
        
        # extract results    
        Yre, x,y,v,a,theta,w,l = self.unpack2(res,N,dt,w,l)
        ...
        minimizer_kwargs = {"method":"L-BFGS-B", "args":(Y1,N,dt,notNan,w,l,*lams),'bounds':bnds,'options':{'disp': False}}
        res = basinhopping(self.obj2, X0, minimizer_kwargs=minimizer_kwargs,niter=niter)
        print(res.fun/lams[0], res.message)
        Yre, x,y,v,a,theta,w,l = self.unpack2(res,N,dt,w,l)
...
if __name__ == "__main__":
    ...
    # re.df = utils.read_data(r"E:\I24-postprocess\benchmark\TM_200_pollute_DA_RE.csv")
    #%%
    # re.postprocess(REMOVE_COLLISION=False, 
    #                 EXTEND = True,
    #                 SAVE = r"E:\I24-postprocess\benchmark\TM_1000_pollute_DA_RE_p.csv"
    #                 # SAVE = ""
    #                 )
    ...
    # # %%
    car = re.original[re.original["ID"]==13][0:300]
    # print(re.estimate_dimension(car))
    carre = car.copy()   
    carre = re.rectify_single_camera(carre, (100,0,0,0,0,0))
    # vis.plot_track_compare(car,carre)
    vis.dashboard([car,carre],["original","rectified"])
    plt.scatter(car["Frame #"].values, car.bbr_x.values-carre.bbr_x.values)