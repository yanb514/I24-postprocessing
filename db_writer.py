...
class DBWriter:
    ...
    def write_fragment(self, **kwargs):
        """
        ...
        self.db[db_parameters.RAW_COLLECTION].insert_one(doc)
        
        
    def write_fragment_multithread(self, **kwargs):
        """
        Write a raw trajectory according to the data schema, found here:
            https://docs.google.com/document/d/1xli3N-FCvIYhvg7HqaQSOcKY44B6ZktGHgcsRkjf7Vg/edit?usp=sharing
        Values that are in the schema, but assigned by the database are: db_write_timestamp, _id.
        ...
        # fire off a thread
        t = Thread(target=self.thread_insert, args=(self.db[db_parameters.RAW_COLLECTION], doc))
        t.daemon = True
        t.start()
