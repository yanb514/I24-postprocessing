...
# start transform
config="logfile /local/transform_${TIME}.log
logfile flush 5
log on
logtstamp after 20
...
echo "$config" > /tmp/postproc-log.conf
screen -c /tmp/postproc-log.conf -dmSL transform-srv /remote/i24_code/I24-postprocessing/start_transform.sh
rm /tmp/postproc-log.conf
