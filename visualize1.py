...
class OverheadCompare():
    ...
    def animate(self, save = False, extra=""):
        ...
        self.cache_vehicle = LRUCache(200*num)
        self.cache_colors = LRUCache(200*num)
...
def dummy(frame, doc, axs):
    if axs is not None:
        print(frame)
    
    
# @catch_critical(errors = (Exception))
def update_cache(frame, doc0, list_dbr, list_veh, cache_colors, cache_vehicle, axs):
    """
    Returns
    -------
    frame : an integer indicating the index of frames
    time_cursor: pymongo cursor turned to list
    o: OverheadCompare object (for pickle)
    axs: matplotlib canvas with only initialized items
    save: True/False
    """
    # Stop criteria
    # print(frame)
    ...
    # GT
    traj_cursor = list_veh[0].collection.find({"_id": {"$in": doc0["id"]} }, 
    ...
    for i,doc in enumerate(docs):
        if "dimensions" not in doc:
            traj_cursor = list_veh[i+1].collection.find({"_id": {"$in": doc["id"]} }, 
                                       {"width":1, "length":1, "coarse_vehicle_class": 1})
            # add vehicle dimension to cache
            for traj in traj_cursor:
                # print("** in curosr")
    ...
    plt.savefig("_temp/{}.png".format(str(frame).zfill(3)))
    plt.clf()
    # return axs

def savefigs(o, extra):           

    now = datetime.utcfromtimestamp(int(time.time())).strftime('%Y-%m-%d_%H-%M-%S')
    file_name = now+"_" + o.list_veh[2].collection._Collection__name +extra+".mp4"
    print(file_name)
    
    
    files = glob.glob('_temp/*')
    for f in files:
        os.remove(f)
    
        # p.map(update_cache, range(frames))
        # p.map(update_cache, args=(frame, time_cursor, o, axs, save,))
    pool = Pool(5)
    time_cursor = o.list_dbr[0].get_range("timestamp", o.t_min, o.t_max)
    # frames=int(o.t_max-o.t_min)*o.framerate
    q = mp.Queue()
    res=[]
    ...
    while not q.empty():
        doc = q.get()
        res.append(pool.apply_async(update_cache, args=(frame, doc, o.list_dbr, o.list_veh, o.cache_colors, o.cache_vehicle, o.axs,)))
        # res.append(pool.apply_async(dummy, args=(frame, doc, o.axs,)))
        frame += 1
    # pool.map(update_cache,((frame, doc, o, o.axs,))
    # for frame in range(frames):
        # doc = time_cursor[frame]
        # print(frame)
        
        # update_cache(frame, doc, o, o.axs)
        # 
    [r.get() for r in res]
    pool.close()
    pool.join()
...
if __name__=="__main__":
    ...
    main(rec = "morose_caribou--RAW_GT1__escalates", save=True, duration=1)
